...
\commentOC{Je me permets de modifier les conséquences du type "happy" en conséquences qui décrivent des états objectifs, du type "mouillé". Je suis nettement plus à l’aise si dans nos exemples les conséquences sont objectives. Je trouve que ça sépare plus sainement les aspects objectifs des aspects subjectifs (confinés à la fonction d’utilité). J’espère que ça ne te posera pas problème. Par ailleurs, je serais en faveur de laisser tomber la suite de l’exemple, par parallèle avec l’exemple MCDM d’une part, et par simplification. Je l’ai donc commenté. Sinon, je trouve qu’on se demande d’où viennent les utilités. Et on ne veut sûrement pas complexifier l’exemple au point de décrire comment on aurait pu éliciter ces informations.}
...
%		Assuming that $p(A)=0.2, p(D)=0.6$, $p(B)=p(C)=0.1$, and that $u_1(c_i)=i-2$, we then have
...
\commentSD{Raccourcir/enlever des citations?}\commentOC{bof, je trouve que c’est justifié de mettre quatre citations sur les aspects philo dans notre papier qui tente de prendre un peu de recul sur ces notions. (Et j’aime bien ces papiers !)}
...
\commentOC{Tu avais raccourci ce §. Il était effectivement trop long, mais je trouve que tu as trop raccourci. Je me permets de rétablir un peu de l’argument précédent, vu qu’on a un peu de place libre. Si la raison de ton raccourcissement n’était pas purement de gagner de la place mais d’évacuer des propos qui te semblaient bateau / inintéressant / faux, n’hésite pas à m’en faire part !}
...
\commentOC{Même remarque que ci-dessus. Me dire si tu as raccourci pour des raisons autres que gain de place.}
...
\commentOC{À la réflexion, ça me semble plus intuitif en commençant par les meilleures conséquences, et avec l’exigence que les probas pour les meilleures conséquences soient les plus élevées pour la meilleure lotterie. Mais n’hésite pas à re-changer si tu n’es pas d’accord.}
...
\begin{example}
	\commentOC{Décider de ce qu’on fait de cet exemple en fonction de ce qu’on fait de \cref{exm:DMU} !}
	Going back to \cref{exm:DMU}, we have that 
