...
\begin{document}
	...
	\commentOC{En lisant mieux, je me rends compte que tu as raison, ta formulation englobe effectivement le cas dont je voulais parler (désolé pour la confusion). Je me permets simplement de noter $u_1$ ton $a$ et $u_1(x(s))$ au lieu de $a(s)$ qui me semble un peu perturbant (vu que la fonction est définie sur $C$ d’une part et que je trouve bizarre qu’elle ne fasse pas apparaitre $x$ dans sa notation alors qu’elle en dépend). Et je reformule légèrement pour pouvoir dire ensuite que ces fonctions sont parfois déduites de $\succeq$.}
	...
	\commentOC{À mon avis, pour réduire le risque de confusion, on ferait bien de choisir deux exemples assez différents, un pour illustrer le MCDM (c’était mon idée derrière ces légumes), et un autre pour illustrer la décision dans le risque. Je suis d’accord que le même contexte peut très bien être vu des deux façons, mais je crains que ça crée des confusions, ou des interrogations (le lecteur peut par exemple se demander comment on va faire dans le contexte MCDM pour gérer l’incertitude illustrée dans le cas DMU sur le gout des aliments, ce qui nous obligerait à introduire un contexte MCDM+DMU, mais je ne pense pas qu’on veuille entrer dans des complexités pareilles). Mais si tu insistes on peut laisser comme ça. Ou voir ça plus tard.}
	\commentSD{Ok, je changerai après pour le contexte, je garderai cependant ton exemple à quatre valeurs distinctes!}
	...
	\commentOC{Je me dis maintenant qu’on devrait laisser le polissage de cette partie pour plus tard : on a intérêt à voir d’abord la place qui nous reste pour savoir à quel niveau de détail on va parler de ces approches.}
	...
	Robust methods in MCDM exist that propose to distinguish conclusions that hold for sure, given some information about the preferences of the DM, and conclusions that possibly hold. Such robust methods typically start from a class of possible models, that represents an assumption about the way the \ac{DM} reasons in problem considered. (For readers used to the machine learning terminology, this corresponds to the version space.) Consider for example that the only thing you know about the \ac{DM} is that she prefers $x = (0, 0, 2)$ to $y = (0, 4, 0)$, and you assume the model is separable \commentOC{Il faudrait sans-doute qu’on explique ça brièvement qqpart.} We may then conclude that $a = (3, 0, 2)$ is preferred to $b = (3, 4, 0)$. But we ignore whether $c = (1, 1, 1)$ is preferred to $d = (0, 2, 2)$. Generally, a robust method, given a class of models $M$ that contains all preference relations considered possible a priori, and a set of constraints $N$ (such as examples of comparisons), will consider that $a$ is necessarily preferred to $b$, $a \succeq^N b$, whenever $a \succeq b$ for all relations $\succeq$ in $M$ that satisfy $N$. \citep{greco… } Such approach permits to represent incomparabilities. They are used to represent incomparabilities that stem from incomplete knowledge of the analyst, rather than to represent intrinsic incomparabilities.
	...
	* Other MCDM model that induce incomplete orders (CP-net... others?)
	...
	\citet{ok_2002} represents a possibly incomplete preference relation using a vector-valued utility function, instead of a real-valued one. This idea is exploited and extended in \citet{eliaz_indifference_2006}, which also cover the DMR setting.
	
	\citet{dubra_expected_2004} represents preferences over lotteries by means of a set of real-valued utility functions. Preference holds whenever the expected utility for the preferred alternative is higher, for all utility functions in the set. (This assumes restrictions over the space of lotteries considered.)
