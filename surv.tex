...
\begin{document}
	...
	Some approaches in MCDM in the family of outranking methods permit to represent incomparabilities. As an example, let us consider the ELECTRE method. It aims at defining a preference relation $S$ on $\allalts$ that…
	...
	* MCDM model that induce incomplete orders (CP-net... others?)
	...
	As recalled in Section~\ref{sec:intro}, probability theory and expected utility are the most widely used tools when having to decide under uncertainty, and naturally induce completeness of preferences under uncertainty. While we mentioned several approaches that tries to depart from this setting while preserving preference completeness, there are only a few that tries to relax the completeness assumption. 
